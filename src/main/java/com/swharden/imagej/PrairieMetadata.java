/* This file was obtained from the following URL on 2022-06-09:
 * https://github.com/ome/bioformats/blob/master/components/formats-gpl/src/loci/formats/in/PrairieMetadata.java
 * 
 * Modified by Scott Harden to:
 *   - Cut dependency on ome.units
 * 
 * Original copyright:
 * 
...
public class PrairieMetadata {
    ...
    private final HashMap<Integer, Sequence> sequences = new HashMap<Integer, Sequence>();
     ...
     */
    public PrairieMetadata(final Document xml, final Document cfg, final Document env) {
        if (xml != null)
            parseXML(xml);
        if (cfg != null)
            parseCFG(cfg);
    ...
    private void parsePVStateShard(final Element el, final ValueTable table) {
        ...
        for (int k = 0; k < svNodes.getLength(); k++) {
            ...
            for (int i = 0; i < ivNodes.getLength(); i++) {
                ...
                if (index == null)
                    continue; // invalid <IndexedValue> element
            ...
            final NodeList sivNodes = keyElement.getElementsByTagName("SubindexedValues");
    ...
    public class Sequence {
         ...
         * frame's {@code index}.
         */
        private final HashMap<Integer, Frame> frames = new HashMap<Integer, Frame>();
         ...
         */
        public Value getValue(final String key) {
            if (sequenceValues.containsKey(key))
    ...
    public class Frame {
         ...
         */
        private Double toLength(Double position, boolean isInvert) {
            if (position == null) {
                return null;
            }
            if (isInvert) {
         ...
         */
        public Value getValue(final String key) {
            if (frameValues.containsKey(key))
     ...
     * <Key key="positionCurrent_ZAxis" permissions="Write, Save" value=
    "-9,62.45" />
     * </pre>
     * <p>
